...
int main(int argc, char ** argv) {
	...
	MPI_Request prev_req[6];
	MPI_Status prev_status[6];
	MPI_Request current_req[2];
	MPI_Status current_status[2];
	int count_prev_req = 0;
	int count_current_req =0;
...
#define T 256
		for (t=0;t<T;t++) {
			...
			if (north != MPI_PROC_NULL){
				MPI_Isend(&u_previous[1][1], local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &prev_req[count_prev_req]);
            			MPI_Irecv(&u_current[0][1], local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &prev_req[count_prev_req+1]);
				count_prev_req += 2;
			}

			// Communicate with west
			if (west != MPI_PROC_NULL){
				MPI_Isend(&u_previous[1][1], 1, column, west, 0, MPI_COMM_WORLD, &prev_req[count_prev_req]);
            			MPI_Irecv(&u_current[1][0], 1, column, west, 0, MPI_COMM_WORLD, &prev_req[count_prev_req+1]);
				count_prev_req += 2;
			}

			// Communicate with south
			if (south != MPI_PROC_NULL){
				MPI_Irecv(&u_previous[local[0]+1][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &prev_req[count_prev_req] );
				count_prev_req ++;
			}

			// Communicate with east
			if (east != MPI_PROC_NULL){
				MPI_Irecv(&u_previous[1][local[1]+1], 1, column, east, 0, MPI_COMM_WORLD, &prev_req[count_prev_req] );
				count_prev_req ++;
			}

			// Wait for the communication to be complete
			MPI_Waitall(count_prev_req,prev_req,MPI_STATUSES_IGNORE);	
			...
			// Communicate with south
			if (south != MPI_PROC_NULL){
				MPI_Isend(&u_current[local[0]][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &current_req[count_current_req]);
				count_current_req ++;
			}

			// Communicate with east
			if (east != MPI_PROC_NULL){
				MPI_Isend(&u_current[1][local[1]], 1, column, east, 0, MPI_COMM_WORLD, &current_req[count_current_req]);
				count_current_req ++;
			}

			// Wait for the communication to be complete
			MPI_Waitall(count_current_req,current_req,MPI_STATUSES_IGNORE);	
