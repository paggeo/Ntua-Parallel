...
#define T 256
		for (t=0;t<T;t++) {
			...
			if (north != MPI_PROC_NULL){
				...
				MPI_Isend( &u_previous[1][1], local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_previous[0][1], local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with south
			if (south != MPI_PROC_NULL){
				MPI_Isend( &u_previous[local[0]][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_previous[local[0]+1][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with east
			if (east != MPI_PROC_NULL){
				MPI_Isend( &u_previous[1][local[1]], 1,column, east, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_previous[1][local[1]+1], 1, column, east, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with west
			if (west != MPI_PROC_NULL){
				MPI_Isend( &u_previous[1][1], 1, column, west, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_previous[1][0], 1, column, west, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			MPI_Waitall(req_len,req,MPI_STATUSES_IGNORE);
			/*Add appropriate timers for computation*/
			gettimeofday(&tcs1, NULL);
			...
			gettimeofday(&tcf1, NULL);

			tcomp += (tcf1.tv_sec-tcs1.tv_sec)+(tcf1.tv_usec-tcs1.tv_usec)*0.000001;


			/*Give me the current values*/
			req_len = 0;
			// Communicate with north
			if (north != MPI_PROC_NULL){
				
				MPI_Isend( &u_current[1][1], local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_current[0][1], local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with south
			if (south != MPI_PROC_NULL){
				MPI_Isend( &u_current[local[0]][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_current[local[0]+1][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with east
			if (east != MPI_PROC_NULL){
				MPI_Isend( &u_current[1][local[1]], 1,column, east, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_current[1][local[1]+1], 1, column, east, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with west
			if (west != MPI_PROC_NULL){
				MPI_Isend( &u_current[1][1], 1, column, west, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_current[1][0], 1, column, west, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			MPI_Waitall(req_len,req,MPI_STATUSES_IGNORE);
			
			gettimeofday(&tcs2, NULL);
			...
			gettimeofday(&tcf2, NULL);

			tcomp += (tcf2.tv_sec-tcs2.tv_sec)+(tcf2.tv_usec-tcs2.tv_usec)*0.000001;
			/*Compute and Communicate*/
