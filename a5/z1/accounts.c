...
typedef struct {
	...
	/*	CPU can access 8 bytes (64 bits) simultaneously. If a struct 
		is not in the same 64 bit block inorder to access it CPU must 
		bring 2 blocks. So we use padding to make sure each struct can
		 be accesed in one CPU cycle. We sacrifice space for performance.
	*/
