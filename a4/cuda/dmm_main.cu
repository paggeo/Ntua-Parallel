...
static void report_transfer(float time, const size_t M, const size_t N,
			    const size_t K) {

  size_t bytes = (M*K + K*N + 2*M*N)*sizeof(value_t);

  printf("Transfer time: %lf ms\n", time);
  printf("Performance:	%lf Gbytes/s\n", bytes * 1.e-6 / time);
}
...
// ------------------
  ...
  cudaEventRecord(stop);
  cudaEventSynchronize(stop);
  cudaEventElapsedTime(&return_time, start, stop);
 
  float total_time = copy_time + return_time;
  report_transfer(total_time, orig_M, orig_N, orig_K);
